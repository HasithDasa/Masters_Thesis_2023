...
def plot_image_and_transition(img, pos, title="Image and Transition"):
    ...
    # For values higher than the boundary index, set to 0 only if they are not already 0
    img_mask_turb[int(pos):, :] = np.where(img_mask_turb[int(pos):, :] != 0, 1, img_mask_turb[int(pos):, :])
    # For values lower than the boundary index, set to 255 only if they are not already 0
    ...
    output_dir_file_name_mask_turb_npy = os.path.join(output_dir, output_file_name_mask_turb)

    output_file_name_mask_lami = file_name.replace('.npy', '_lami.npy')
    output_dir_file_name_mask_lami_npy = os.path.join(output_dir, output_file_name_mask_lami)
