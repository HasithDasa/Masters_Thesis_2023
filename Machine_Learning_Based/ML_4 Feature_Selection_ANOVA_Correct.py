...
def calculate_glcm_features_on_patch(patch):
    ...
    return np.hstack([energy, contrast, homogeneity]).flatten()
...
for start_col in range(complete_start_column, complete_end_column, patch_size_cols):
    ...
    X_ANOVA = df.iloc[:, :-1]  # Features
    y_ANOVA = df.iloc[:, -1]  # Target
    ...
    X_new = select_k_best.fit_transform(X_ANOVA, y_ANOVA)
