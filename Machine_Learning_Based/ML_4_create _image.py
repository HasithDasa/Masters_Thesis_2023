...
def crop_image_based_on_zeros(img):
    ...
    # Crop the image
    cropped_img = img[int(params["te"]):int(params["le"]), :]
    return cropped_img
...
def plot_image_and_save(img, pos, pos_from_trailing, title="Image and Transition"):
    ...
    file_path = "D:/Academic/MSc/Thesis/Project files/Project Complete/data/new data/save_images/image_with_trans_line/new_data_set/231002_170018/normalized/validation/trans_details.xlsx"
    ...
    if os.path.exists(file_path):
        ...
        df_data_save = pd.DataFrame(columns=['image name', 'transitional_line', 'trailing_edge'])

    # Append the new data
    df_data_save = df_data_save.append(
        {'image name': file_name, 'transitional_line': value, 'trailing_edge': trailing_edge_pos}, ignore_index=True)

    # Save the DataFrame to the Excel file
    df_data_save.to_excel(file_path, index=False)

    output_dir_files = "D:/Academic/MSc/Thesis/Project files/Project Complete/data/new data/save_images/image_with_trans_line/new_data_set/231002_170018/normalized/validation"
    output_dir_masks = "D:/Academic/MSc/Thesis/Project files/Project Complete/data/new data/save_images/image_with_trans_line/new_data_set/231002_170018/normalized/masks"
    ...
    # normalized_image = (cropped_img - min_val) / (max_val - min_val)

    mean_val = np.mean(cropped_img)
    std_val = np.std(cropped_img)
    normalized_image = (cropped_img - mean_val) / std_val

    np.save(output_dir_file_name_npy, normalized_image)
    ...
    cropped_mask_turb = crop_image_based_on_zeros(img_mask_turb)
    ...
    cropped_mask_lami = crop_image_based_on_zeros(img_mask_lami)
