...
params = {
    ...
    "file_name"         : "irdata_0001_0025",
    "type"              : ".npy",
    "min_arc_length"    : 100,
    "noise_std"         : 0, 
    "data_date"         : "231508",
    "folder_path_save"  : r"D:\Academic\MSc\Thesis\Project files\Project Complete\data\new data\save_images",
    "type_save"         : ".png"
    }
...
def main():
    ...
    com_file_name = params['file_name'] + params['type']
    file_str = os.path.join(params['folder_path'], com_file_name)
...
def save_image(img, params=params):
    ...
    normalized_image = cv2.normalize(img, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

    # Convert to uint8
    uint8_image = img.astype(np.uint8)
